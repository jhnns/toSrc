 ...
 * 
 * @author Johannes Ewald http://github.com/jhnns/
 * @version 0.1.0
 */
 ...
 */
function toSourceRecursive(obj, depth) {
    var typeOfResult;
        ...
        if(obj instanceof Array) {
            if(depth > 0) {
                ...
                for(i=0; i<obj.length; i++) {
                    objString += toSourceRecursive(obj[i], depth - 1) + ', ';
                }
                ...
                if(depth > 0) {
                    ...
                    for(key in obj) {
                        if(obj.hasOwnProperty(key)) {
                            objString += '"' + key + '": ' + toSourceRecursive(obj[key], depth - 1) + ', ';
                        }
                    }
                    if(objString.length > 1) {
                        objString = objString.substring(0, objString.length - 2);
...
function toSource(obj, depth) {
    var result = toSourceRecursive(obj, depth);
